 ...
 */
public class AppSnd extends Thread implements AppFeatures, InitFeatures, DataMapFeatures {
    ...
    private byte[] buildHeader(byte FC, byte group, byte variation) {
        ...
        header[0] = (byte) (0xc0 + ((appRcv.getConfig().isREQ_APP_CONFIRM()) ? 0x20 : 0)); // AC
     ...
     */
    public Buffer buildRequestMsg(byte FC, byte group, byte variation, int start, int stop) throws Exception {
        Buffer requestFrame = new Buffer(S);
        requestFrame.writeBytes(buildHeader(FC, group, variation));
     ...
     * in database
     */
    public Buffer buildRequestMsg(byte FC, byte group, byte variation, int quantity, boolean withData) {
        Buffer requestFrame = new Buffer(S);
        requestFrame.writeBytes(buildHeader(FC, group, variation));
        ...
        if (withData) {
            if (group == 50) {
                requestFrame.writeBytes(DataObject.getTime(System.currentTimeMillis()));
            }
            // else {
     ...
     * CONTROL function
     */
    public Buffer buildRequestMsg(byte FC, byte group, byte variation, int[] values, boolean withData) {
        Buffer requestFrame = new Buffer(S);
        requestFrame.writeBytes(buildHeader(FC, group, variation));
     ...
     */
    public Buffer addObjectToRequest(Buffer requestFrame, byte FC, byte group, byte variation, int start, int stop,
     ...
     * with multiple objects
     */
    public Buffer addObjectToRequest(Buffer requestFrame, byte FC, byte group, byte variation) {
        if (requestFrame.length() == 0) {
            requestFrame.writeBytes(buildHeader(FC, group, variation));
        }
        else {
    ...
    private void buildApplicationMsg(byte[] someBytes) throws Exception {
        ...
        if (DEBUG) {
            System.out.println("[ApplicationLayer] Sending frame" + Utils.Display(frameRcv.value()));
        }
